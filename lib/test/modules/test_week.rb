...
class TestWeek < Test::Unit::TestCase
  ...
  def test_week_of_month
    [Date,Time].each do |klass| 
      ...
      assert_equal 2, klass.new(2013,1,6).week_of_month

      assert_equal 3, klass.new(2013,1,13).week_of_month
    end
  end
  ...
  def test_general_week_of_month
    [Date,Time].each do |klass|
      ...
      assert_equal 0, klass.new(2013,1,1).general_week_of_month

      assert_equal 0, klass.new(2013,7,1).general_week_of_month
    end
  end



  def test_monday_configured_week_split
  ...
  def test_first_week?
    [Date,Time].each do |klass|
      ...
      assert klass.new(2012,1,2).first_week?
    end
  end
  ...
  def test_monday_configured_second_week?
    WeekOfMonth.configuration.monday_active = true
    [Date,Time].each do |klass|
      assert !klass.new(2013,1,6).second_week?
      assert !klass.new(2013,1,2).second_week?
  ...
  def test_monday_configured_week_of_month
    WeekOfMonth.configuration.monday_active = true
    [Date,Time].each do |klass| 
      assert_equal 5, klass.new(2013,1,31).week_of_month

      assert_equal 1, klass.new(2013,1,6).week_of_month

      assert_equal 2, klass.new(2013,1,13).week_of_month
    end
    WeekOfMonth.configuration.monday_active = false
  end

  def test_monday_configured_general_week_of_month
    WeekOfMonth.configuration.monday_active = true
    [Date,Time].each do |klass|
      assert_equal 4, klass.new(2013,1,31).general_week_of_month
      ...
      assert_equal 1, klass.new(2013,7,1).general_week_of_month
    end
    WeekOfMonth.configuration.monday_active = false
  end
  ...
  def test_monday_configured_first_week?
    WeekOfMonth.configuration.monday_active = true
    [Date,Time].each do |klass|
      assert klass.new(2012,1,1).first_week?
      assert !klass.new(2012,1,31).first_week?
